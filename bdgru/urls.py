...
urlpatterns = [
    ...
    url(r'^campaign/(?P<id>[0-9]+)/$', views.CampaignView.as_view(template_name=t('campaign.html')),
        name=views.CampaignView.view_name),
    url(r'^campaign/(?P<id>[0-9]+)/(?P<action>(update|del|ready|start|done|renew|track|add))/$', views.CampaignView.as_view(
    ...
    url(r'^campaign/(?P<action>new)/$', views.CampaignView.as_view(template_name=t('campaign.html')),
        name=views.CampaignView.view_name),
    url(r'^campaign/(?P<id>[0-9]+)/(?P<action>edit)/$', views.CampaignView.as_view(template_name=t('campaign-edit.html')),
        name=views.CampaignEditView.view_name),
